 ...
 */
async function processPropertyStep2Only(id: number, outcode: string, incode?: string): Promise<AddressResult> {
  const propertyId = id.toString();
  const input: PropertyInput = { propertyId, outcode, incode };
